...
async def _on_startup(app: web.Application) -> None:
    ...
    # Миграция view: парсерские данные живут 24ч вместо 2ч
    try:
        await db._execute("""
        ...
        """)
        logger.info("View migration: parser data lifetime 2h -> 24h")
    except Exception as e:
        logger.warning(f"View migration skipped: {e}")
    ...
    texts = [
        {
            "platform": "Telegram",
            "text": (
                f"🚗 Покажу как я экономлю на бензине 💰\n\n"
                f"Приложение «Бензин рядом» показывает где заправляться дешевле в моём городе. "
                f"Я за месяц нашёл 3 АЗС где дешевле на 5-7₽/литр — экономия 1500-2000₽.\n\n"
                f"Бесплатно, без рекламы, просто работает. Попробуй 👇\n"
        ...
        },
        {
            "platform": "VK",
            "text": (
                f"Ребята, нашёл тему — приложение для экономии на бензине ⛽\n\n"
                f"«Бензин рядом» показывает где в твоём городе сейчас заправляться выгоднее. "
                f"Я уже месяц пользуюсь — реально экономлю 1000-2000₽/мес. Можно.compare цены прямо в уме.\n\n"
                f"Ссылка: https://vk.com/benzyn_ryadom?start=ref_{code}"
            ),
        },
        {
            "platform": "WhatsApp",
            "text": (
                f"Привет! 🚗\n\n"
                f"Нашёл приложение «Бензин рядом» — показывает где бензин дешевле рядом. "
                f"Реально удобно, экономлю каждый месяц. Попробуй 👇\n"
                f"https://t.me/benzyn_ryadom_bot?start=ref_{code}"
            ),
        },
        {
            "platform": "Друзьям лично",
            "text": (
                f"Слушай, тут тема для водителей — приложение «Бензин рядом» 🚗\n\n"
                f"Показывает где бензин дешевле в твоём городе, можно.compare АЗС и сэкономить. "
                f"Я сам пользуюсь, экономия реальная.\n\n"
                f"Плюс — за каждый Premium друг получает 15% скидку, а мне 50% от платежа. "
                f"Так что если реально будешь пользоваться — мне тоже бонус 🎁\n\n"
                f"Ссылка: https://t.me/benzyn_ryadom_bot?start=ref_{code}"
            ),
        },
        {
            "platform": "🚀 Заработок",
            "text": (
                f"💸 Хочешь зарабатывать без вложений?\n\n"
                f"Рассказываю как: приложение «Бензин рядом» платит за каждого друга, "
                f"которого привёл и он купил Premium.\n\n"
                f"📊 Цифры:\n"
                f"• 1 друг = 125₽ (50% от Standard 250₽)\n"
                f"• 5 друзей = 625₽\n"
                f"• 10 друзей = 1250₽\n"
                f"• 50 друзей = 6875₽ (уже 55%)\n"
                f"• 200 друзей = 16 250₽ (65%)\n\n"
                f"🎯 Без потолка. Вывод от 100₽ на карту.\n\n"
                f"👉 Начни: https://t.me/benzyn_ryadom_bot?start=ref_{code}"
            ),
        },
        {
            "platform": "💡 Истории успеха",
            "text": (
                f"💡 Реальные кейсы наших рефереров:\n\n"
                f"👤 Алексей из Москвы — привёл 30 друзей за 2 месяца → заработал 4500₽ на оплату телефона и интернета\n\n"
                f"👤 Марина из Питера — её брат работает таксистом, рассказал коллегам → 12 оплат в первый месяц → 1500₽\n\n"
                f"👤 Дмитрий — админ чата водителей ВК (5000 участников) → 7 оплат → 875₽, и это только начало\n\n"
                f"У всех была одна и та же стартовая точка: просто начать. 🚀\n\n"
                f"👉 Твоя ссылка: https://t.me/benzyn_ryadom_bot?start=ref_{code}"
            ),
        },
    ]
