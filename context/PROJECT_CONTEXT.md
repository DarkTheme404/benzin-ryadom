...
## Flutter приложение — Авторизация

### Регистрация
1. Ввод: **Имя** + **Пароль** (мин 4 символа) + **VK ссылка** (опционально) + **TG ссылка** (опционально)
2. Backend хеширует пароль (hashlib pbkdf2_hmac)
3. Если указана VK ссылка → ищет существующий VK-аккаунт по `screen_name`
4. Возвращает `user_id` (internal) + `telegram_id` (для API запросов)
5. Flutter сохраняет `telegramId` в SharedPreferences → шлёт как `telegram_id` параметр

### Вход
1. Ввод: **Имя** + **Пароль**
2. Backend ищет по имени, проверяет bcrypt-хеш
3. Возвращает `user_id` + `telegram_id`

### Выход
- Кнопка «Выйти» в профиле → очищает SharedPreferences → возвращает на экран регистрации
...
- **Реферер** получает 50% комиссии (только для **Элит+** тарифов)
